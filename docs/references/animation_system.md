...
- **Fidget** — a short additive gesture chosen by a **fidget selector** ([advanceFidget](../../src/main/java/dev/breezes/settlements/domain/animation/DefaultIdleLifeAnimator.java:93)): a 600–1500 tick timer (≈30–75 s) with weighted-random, **no-repeat** selection ([nextFidgetIndex](../../src/main/java/dev/breezes/settlements/domain/animation/DefaultIdleLifeAnimator.java:113)), seeded by entity id (`entityId · 31 + 17`) so villagers desync. A fidget only *starts* when `actionActive()` is false, and any **in-flight fidget is cancelled the instant an action begins** — idle-life composites *under* the action, which rarely authors the same body/eye targets, so a lingering fidget would otherwise keep tugging those bones against the work animation. After an action the timer is held out a further `MIN_FIDGET_DELAY_TICKS`, so finished work is never immediately followed by a fidget.

One fidget ships today: `ponder()` — the villager furrows its brow, casts its eyes down and aside, blinks, and shifts its weight in a small double-bob. It deliberately does **not** drive `HEAD_ROTATION_OVERRIDE`: an absolute head seize starting from a neutral forward pose would snap the head away from (and back to) whatever the villager was actually looking at, so the gaze is left to vanilla look-tracking and the "thinking" read is carried by the eyes, brow, and body.

> **First cut, with stubs.** The idle-life clips are explicitly placeholder stubs ([IdleLifeAnimations](../../src/main/java/dev/breezes/settlements/domain/animation/IdleLifeAnimations.java) header comment). Two channels from the original design are **not yet wired**: the **expression channel** (face presets on `MONOBROW`/`MOUTH`) and the **`Mood`** value that was meant to bias which expression/gesture is chosen. The face targets exist (see [target catalog](#target-catalog)) but no clip drives `MOUTH` (the `ponder` fidget does nudge `MONOBROW`). Fidget gating is on action presence only — a fidget will not start, and an in-flight one is cancelled, whenever the action layer is occupied; there is no separate speed gate (idle-life is faded by locomotion weight in the stack instead).
