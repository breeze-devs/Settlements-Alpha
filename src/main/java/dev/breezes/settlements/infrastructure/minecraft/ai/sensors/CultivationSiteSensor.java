...
/**
 * Discovers nearby Cultivation Lilies and records the valid ones in memory.
 * <p>
 * Lilies are block entities, so this iterates each loaded chunk's block-entity map rather than
 ...
 * <p>
 * Only the {@code valid} flag is checked here; whether a lily currently has actionable work is left
 * to the behavior's {@code NEEDS_WORK} precondition so the memory stays a stable "known lilies" list
 * rather than churning as cells are tilled and planted.
 */
public class CultivationSiteSensor extends Sensor<Villager> {
    ...
    public CultivationSiteSensor() {
        super(SCAN_INTERVAL_TICKS);
    }

    @Override
    public Set<MemoryModuleType<?>> requires() {
        return Set.of(MemoryTypeRegistry.CULTIVATION_SITES.getModuleType());
    }
    ...
    protected void doTick(@Nonnull ServerLevel level, @Nonnull Villager villager) {
        ...
        for (int cx = minChunkX; cx <= maxChunkX; cx++) {
            for (int cz = minChunkZ; cz <= maxChunkZ; cz++) {
                ...
                for (Map.Entry<BlockPos, BlockEntity> entry : chunk.getBlockEntities().entrySet()) {
                    ...
                    // The valid flag is cached by the lily's server tick — reading it is O(1).
                    if (!(entry.getValue() instanceof CultivationLilyBlockEntity lily) || !lily.isValid()) {
                        continue;
                    }
        ...
        if (found.isEmpty()) {
            baseVillager.getBrain().eraseMemory(MemoryTypeRegistry.CULTIVATION_SITES.getModuleType());
        } else {
            baseVillager.getBrain().setMemory(MemoryTypeRegistry.CULTIVATION_SITES.getModuleType(), List.copyOf(found));
        }
    }
