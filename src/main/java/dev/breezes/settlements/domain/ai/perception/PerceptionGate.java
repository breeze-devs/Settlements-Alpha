 ...
 * <p>
 * The gate implements the anti-telepathy rule: a villager only learns from events it could
 * plausibly have perceived. Admission is a Manhattan chunk-distance check — events whose chunk
 * origin is farther than {@link #MAX_PERCEPTION_CHUNK_RADIUS} chunks away are skipped. Chunk
 * coordinates come directly from the event envelope (no world lookup needed), so this check is
 * O(1) and safe on the server thread.
 * <p>
