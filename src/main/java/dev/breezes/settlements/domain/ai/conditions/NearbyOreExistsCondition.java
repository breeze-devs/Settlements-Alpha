...
public class NearbyOreExistsCondition<E extends BaseVillager> extends NearbyBlockExistsCondition<E> {
    ...
    private static boolean isValidOre(BlockPos blockPos, Level level) {
        return level.getBlockState(blockPos.above()).isAir()
                ...
                || level.getBlockState(blockPos.north()).isAir()
                || level.getBlockState(blockPos.south()).isAir()
                || level.getBlockState(blockPos.east()).isAir()
                || level.getBlockState(blockPos.west()).isAir();
    }
}
